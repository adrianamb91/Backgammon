...
class Board: 
    ...
    def make_move(self, col, n) :
        source = self.allSpaces[col]
        if (col > 0 and (source[0] == 0 or self.computer != source[1])) :
            ...
            destination = self.allSpaces[col+n]
            if (destination[1] == self.empty or destination[1] == self.computer) :
                source[0] -= 1
                destination[0] += 1
                destination[1] = self.computer
                if (source[0] == 0) :
                    source[1] = self.empty
            else:
                if (destination[0] > 1) :
